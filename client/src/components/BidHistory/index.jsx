          ...
          <Card.Header as="h5" className="text-center cardTopper">Bid History</Card.Header>
            <Card.Body>
              <Card.Text className="d-flex flex-wrap justify-content-evenly">
                {userData?.bids && userData?.bids.map((bid, index) => {
                  return <Col key={index} xs={6} className="p-4"><span className="p-4 border-start border-end bidHistory">${bid?.amount} on {bid?.product.name}</span></Col>
                })}
              </Card.Text>
            </Card.Body>
          </Card>
